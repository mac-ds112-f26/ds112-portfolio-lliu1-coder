...
Answer: Both qmd and html will be stored in the portfolio, and I think we can do it.
...
`\` normalize special signal.

`![](url)` : Printing the picture of the link instead of the link.
...
Anwser: it wll appear excatly the same in both qmd and Html.
...
```{r}
# temprature changes from C to F
temperature_c <- 10
